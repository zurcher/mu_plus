    ...
    pub fn install(controller: efi::Handle, keyboard_handler: &mut KeyboardHidHandler) -> Result<(), efi::Status> {
        ...
        let status = unsafe {
            static_boot_services().install_protocol_interface_unchecked(
                Some(controller),
                &protocols::simple_text_input::PROTOCOL_GUID,
                simple_text_in_ptr as *mut c_void,
            )
        };
...
mod test {
    ...
    fn install_should_install_simple_text_in_interface() {
        with_global_lock(|| {
            ...
            boot_services.expect_create_event_unchecked().returning(|_, _, _, _: *mut c_void| Ok(4 as efi::Event));
            boot_services.expect_install_protocol_interface_unchecked().returning(|_, protocol, _| {
                assert_eq!(protocol, &protocols::simple_text_input::PROTOCOL_GUID);
                Ok(5 as efi::Handle)
            });
            ...
            SimpleTextInFfi::install(2 as efi::Handle, &mut keyboard_handler).unwrap();
    ...
    fn uninstall_should_uninstall_simple_text_in_interface() {
        with_global_lock(|| {
            ...
            SimpleTextInFfi::uninstall(1 as efi::Handle, 2 as efi::Handle).unwrap();
